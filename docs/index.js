...
$(document).ready(function () {
	...
	$('#canvas').click(function (e) {
		if (computing) {
			return; // if svm is working, unable to push new points.
		}
...
var SVM = function () {
	...
	_createClass(SVM, [{
		...
		key: 'getIllegalValue',
		value: function getIllegalValue(examineAll) {
			var _this2 = this;

			var first = this.index;
			var regularize = function regularize(x) {
				if (x >= _this2.data.length) {
					return x - _this2.data.length;
				}
				return x;
			};
			for (var i = 0; i < this.data.length; i++) {
				if (!examineAll && Math.abs(this.lagrangeMultipliers[i]) < eps && Math.abs(this.lagrangeMultipliers[i] - this.regularization) < eps) {
					continue;
				}
		...
		value: function update(i1, i2) {
			...
			var a2NewUnclipped = (a2 + t2 * (E1 - E2)) / (this.kernel(d1, d1) - 2 * this.kernel(d1, d2) + this.kernel(d2, d2));
			var a2New = a2NewUnclipped;
			if (a2NewUnclipped > H) {
				a2New = H;
			...
			var a1New = a1 + t1 * t2 * (a2 - a2New);

			this.lagrangeMultipliers[i1] = a1New;
			this.lagrangeMultipliers[i2] = a2New;
			// update bias parameter (12.9),(12,10)式
			var b1New = E1 + t1 * (a1New - a1) * this.kernel(d1, d1) + t2 * (a2NewUnclipped - a2) * this.kernel(d1, d2) + this.bias;
			var b2New = E2 + t1 * (a1New - a1) * this.kernel(d1, d2) + t2 * (a2NewUnclipped - a2) * this.kernel(d2, d2) + this.bias;
		...
		value: function learn() {
			...
			for (;;) {
				...
				var i1 = this.getIllegalValue(examineAll);
				if (i1 === -1) {
					if (examineAll) {
						break;
					} else {
